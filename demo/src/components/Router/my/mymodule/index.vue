          ...
          <li v-for="(item,index) in data" :key="index">
            ...
            <div class="list-main clearfix">
              ...
              <div class="operation fr">
                ...
                <a href="javascript:;" class="delete-want" @click="deleted(item._id)">删除发布</a>
              </div>
            </div>
          </li>
        </ul>
...
<script>
    export default {
      ...
      methods:{
        ...
        deleted(id){
          this.$ajax.delete('/shop/delete?id='+id)
            .then(res=>{
              this.$router.push({name:'transition',params:{path:'/user'}})
            })
        }
      },
      created(){
        let useruid = window.localStorage.getItem('useruid')
        this.$ajax('user/myshop?uid='+useruid)
          .then(res=>{
            this.data= res.data
